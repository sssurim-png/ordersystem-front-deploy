<template>
<div>Hook 함수 테스트</div>
<button @click="moveScreen()">다른 화면으로 이동(라우트방식)</button>
<div>{{ count1 }}</div>
<button @click="incrementCount1">count1값 증가</button>
...
// 화면이 열리기전(vue인스턴스가 생성되기전) 실행되는 함수 -axios받아온거
// 예시)목록조회, 상세조회
    created(){
        console.log("created 테스트");
    },
// 컴포넌트가 화면상에서 제거(이동)될 때 실행되는 함수
// 예시)채팅(알림)화면을 벗어나 다른화면으로 이동시에 실시간 연결 종료요청(물리적화면을 종료시키는 경우는 제외)(그냥끄는게 이 함수는 아니다) 
    beforeUnmount(){
        console.log("beforeUnmount 테스트")
    },

    // 특정 변수값의 변경사항을 인지하여 함수를 자동실행하는 hook함수
    // 예시)검색어 입력시 자동완성 api호출
    watch: {
        // 인지하고자하는 변수값과 일치되는 함수명 사용
        count1(){
